...
export const MoveStairTool: React.FC<{
  ...
  useEffect(() => {
    ...
    // We avoid setting isTransient on the store to prevent the stair system
    // from resetting the mesh position (it resets on dirty) and from
    // triggering expensive geometry rebuilds on every frame.
    ...
    const syncHostedPreview = (
      patch: Pick<StairSegmentNode, 'position' | 'rotation'>,
    ) => {
      if (movingNode.type !== 'stair-segment') return
    ...
    // Alignment for the top-level stair only. Segments live in parent-local
    // space (a different frame from the building-local candidate pool / guide
    // layer), so we leave them on the plain grid+corner snap. Stairs align
    // by their footprint bounding-box corners.
    const alignTopLevel = movingNode.type === 'stair'
    ...
    const computeLocal = (
      ...
      // Segments have a transformed parent (the stair). Convert world → parent-local
      ...
      // The stair lives directly in the level — its stored position is building-local.
  ...
  // Show the same green drag box for both the top-level stair and its
  ...
  const showBoundingBox =
    movingNode.type === 'stair' || movingNode.type === 'stair-segment'
